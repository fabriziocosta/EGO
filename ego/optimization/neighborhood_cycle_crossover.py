...
class NeighborhoodCycleCrossover(object):

    def __init__(self, n_neighbors=None, n_cycles=1, n_edges_per_cycle=1, fraction=0.5, part_importance_estimator=None, tournament_size_factor=10, size_n_std_to_accept=None):
        ...
        self.n_edges_per_cycle = n_edges_per_cycle
    ...
    def _get_valid_edges(self, graph, max_num=None, n_edges_per_cycle=None):
        cycles = nx.cycle_basis(graph)
        random.shuffle(cycles)
        if max_num is not None:
            cycles = cycles[:max_num]
        all_two_edges = []
        for cycle in cycles:
            n = len(cycle)
            if n > 3:
                if n_edges_per_cycle is None:
                    for ii in range(n - 1):
                        for jj in range(ii + 1, n):
                            start_endpoints = [ii, jj]
                            end_points = [(start_endpoint + 1) % n for start_endpoint in start_endpoints]
                            two_edges = [(cycle[i], cycle[j]) for i, j in zip(start_endpoints, end_points)]
                            if self._is_valid(two_edges[0][0], two_edges[0][1], two_edges[1][0], two_edges[1][1], graph):
                                all_two_edges.append(two_edges)
                else:
                    for it in range(n_edges_per_cycle):
                        start_endpoints = random.sample(range(n), 2)
                        end_points = [(start_endpoint + 1) % n for start_endpoint in start_endpoints]
                        two_edges = [(cycle[i], cycle[j]) for i, j in zip(start_endpoints, end_points)]
                        if self._is_valid(two_edges[0][0], two_edges[0][1], two_edges[1][0], two_edges[1][1], graph):
                            all_two_edges.append(two_edges)
